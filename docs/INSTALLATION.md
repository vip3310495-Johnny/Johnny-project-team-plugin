...
## 安裝外部 skills 與 OmniParser

此 plugin 不封裝 `grilling`、`codebase-design`、
`improve-codebase-architecture`。請從團隊核准的來源下載，並分別安裝到
`$CODEX_HOME/skills/<skill-name>/SKILL.md`。Phase 0 或 Phase 4 使用相關流程前，
必須先確認對應 skill 存在。

OmniParser 也不隨 plugin 安裝。請依
[Microsoft OmniParser](https://github.com/microsoft/OmniParser) 官方文件另行安裝。
SDD DQA 一律先使用實際截圖；只有證據不清楚時才使用 OmniParser。若當次必要但
尚未安裝，回報 `BLOCKED_DEPENDENCY`，不得降低 UI 驗證標準。

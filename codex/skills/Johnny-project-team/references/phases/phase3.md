...
## 7. Claude DQA 最終抓漏
- TDD 與 SDD 都記錄為 PASS 後，`verify_all_dqa_passed_hook.py` 會自動呼叫外部 Claude CLI 進行最後的獨立抓漏。
- Claude 只能以唯讀工具檢查專案；每次審查最多支出 2 美元、最長 900 秒，並將結構化報告寫入 `Claude DQA/`。
- Claude 回覆 PASS 才會寫入 `.agents/.dqa_status.json`；任何發現、CLI 錯誤、逾時或格式錯誤一律記錄為 FAIL 並維持閘門鎖定。
- 若 Claude 發現問題，必須將問題退回給工程師修正，重跑內部 DQA 後再次觸發三重 DQA 閘門。
...
- 腳本會先驗證視覺化報告與三重 DQA；若 Claude 尚未執行，會自動執行 Claude DQA。全部通過後才會進入 **Phase 4 (成品驗收階段)**。

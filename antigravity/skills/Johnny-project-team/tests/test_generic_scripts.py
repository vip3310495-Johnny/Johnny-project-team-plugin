...
EXCLUDE_FROM_GENERIC = {'path_guard.py', 'git_guard.py', 'setup_hooks.py'}
...
def test_generic_script_execution(script_path):
    basename = os.path.basename(script_path)
    
    # Phase 1: Help Check
    res_help = subprocess.run([sys.executable, script_path, "--help"], capture_output=True, text=True)
    ...
    if basename == 'phase_gate_hook.py':
        cmd = [sys.executable, script_path, "--from_phase", "1", "--to_phase", "2", "--ceo_signature", "/approve"]
    elif basename == 'verify_lesson_hook.py':
        cmd = [sys.executable, script_path, "--role", "PM", "--proposal", "這是一個很棒的測試教訓提案，我們必須遵守SOP防呆防範未來的錯誤發生"]
        
    res_exec = subprocess.run(cmd, capture_output=True, text=True)

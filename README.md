...
> 本工作流程以 **Google Antigravity** 作為核心專案管理與高階決策中樞，並以原生的 `Engineer` 子代理人作為預設開發主力；同時保留 **Claude Code CLI** 作為外部獨立審查 (DQA) 或是進階外包選項 (Advanced Outsourcing)。
> 
> 您只需要扮演 **CEO (決策者)**，向 Antigravity 提出您的商業需求；系統會自動喚醒專案經理 (PM)、架構師 (Architect)、品管測試員 (DQA) 與工程師 (Engineer)，並嚴格按照中大型專案的標準 SOP 進行開發與調度。

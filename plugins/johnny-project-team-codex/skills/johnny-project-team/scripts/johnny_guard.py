...
from johnny_common import STATE_DIR, git_root, is_enabled, read_json, run_git, staged_tree, subject_tree
...
def validate_paths(project: Path, paths: list[str], config: dict, state: dict) -> None:
    ...
    roots = tuple(
        config.get("phase3_commit_roots", ["src/"])
        if phase == 3
        else config.get("phase4_commit_roots", ["src/"])
    )
    if phase == 4 and paths:
        approval = state.get("phase4_execution") or {}
        if approval.get("status") != "APPROVED":
            fail("Phase 4 is still planning; CEO approval through johnny_phase4_start.py is required before construction")
    for value in paths:
        path = PurePosixPath(value)
        if ".." in path.parts or path.is_absolute():
            fail(f"unsafe staged path: {value}")
        if phase in {3, 4} and roots and not value.startswith(roots):
            fail(f"Phase {phase} construction commits may contain only src/ delivery files: {value}")
        if value.startswith(".johnny/") and value not in {
            ".johnny/config.json", ".johnny/state.json", ".johnny/dqa-status.json"
        }:
...
def validate_dqa(project: Path, paths: list[str], config: dict, state: dict) -> None:
    phase = int(state.get("phase", 0))
    configured = config.get("dqa_required_phases")
    required_now = (
        phase >= int(config.get("dqa_required_from_phase", 4))
        if configured is None
        else phase in {int(value) for value in configured}
    )
    if not required_now or not paths:
        return
    roles = ["tdd", "sdd"]
    ...
    if escalation.get("active", False):
        fail(f"Milestone is frozen pending CEO resolution after {escalation.get('role')} DQA rejection {escalation.get('rejection_count')}")
    if status.get("scope", "ticket") != "ticket":
        fail("Phase 3 and Phase 4 construction require ticket-scoped DQA evidence")
    results = status.get("results", {})
    missing = [name for name in roles if results.get(name) != "PASS"]
    if missing:
        fail("required DQA did not pass: " + ", ".join(missing))
    reviews = status.get("reviews", {})
    stale = [name for name in roles if (reviews.get(name) or {}).get("ecc_selection_sha256") != selection_hash]
    if stale:
        fail("DQA review used a different ECC selection: " + ", ".join(stale))
...
def validate_branch(project: Path, config: dict, state: dict, event: str) -> None:
    ...
    if branch == "main" or branch.startswith("feature/"):
        fail(f"direct {event} on protected branch {branch!r} is not allowed; use a controlled PM merge after approval")
    if phase == 3 and not branch.startswith("codex/milestone-"):
        fail("Phase 3 work must use a codex/milestone-Mxx branch")
    if phase == 4 and not branch.startswith("codex/phase4-M"):
        fail("Phase 4 work must use a codex/phase4-Mxx branch")

...
2. **強制記憶橋接 (Memory Bridge)**：必須在指令中要求 Claude 完成任務後，產出一份 `claude_handover_report.md`，並存入專屬的角色資料夾 (若是工程師外包則存入 `/Claude Engineer/`，若是測試則存入 `/Claude DQA/`)。
   *範例指令*：`npx @anthropic-ai/claude-code -p "請根據 PM/PRD.md 實作 src/auth 模組。完成後，必須產出一份報告 claude_handover_report.md 並存入 Claude Engineer 資料夾。"`

## 二、 權限核准攔截 (Permission Escalation) [CRITICAL]

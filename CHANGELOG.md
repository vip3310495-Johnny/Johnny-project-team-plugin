...
本 repository 只維護 Codex Plugin，本檔記錄其版本變更。

## 2.3.0-codex.1 — 2026-08-10

### Changed

- 移除非 Codex Edition，只保留單一 Codex plugin source。
- README 與 Phase 0～5 以原詳細工作流程為內容藍本，全面轉譯為 Codex agents、state、evidence、branches 與 gates。
- Phase 2 只提供 `SUPERVISED`／`AUTONOMOUS` execution policy。
- Phase 4 不再假設存在 `CONTEXT.md`，改由 Architect 產出 `Phase4_Codebase_Context.md`。
- Phase 5 是最終階段。
...
- Model Recommendation Matrix 初始值更新為 PM／Architect `sol (Medium)`、
  Engineer `terra (Medium)`、TDD／SDD DQA `terra (High)`、
  Security DQA `sol (Medium)`、TE `terra (Low)`。
...
- 正式 references 不再調用未實作的 placeholders 或不存在的

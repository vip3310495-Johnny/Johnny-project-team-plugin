...
## 📖 方法工具快速索引 (Method Toolbox)

PM 可依情境使用 User Story、Impact Mapping、MoSCoW、WSJF、Kano、AHP、
PERT、MECE、5 Whys、PDCA、Hick、Fitts、Gestalt 等方法，但它們是分析
框架，不代表 Plugin 內有可執行且已驗證的腳本。正式自動化入口只以主 Skill
的 `references/script-catalog.md` 為準；`experimental/` 下的概念原型不得由
PM、Hook 或工作流呼叫。

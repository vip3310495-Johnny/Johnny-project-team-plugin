...
RULES_V3_MARKER = "<!-- johnny-project-contract-v3 -->"
RULES_V4_START = "<!-- johnny-project-contract-v4:start -->"
RULES_V4_END = "<!-- johnny-project-contract-v4:end -->"
PROCESS_ROOTS = (
    ".johnny/",
    ".agents/",
    ".codex/",
...
RETIRED_MANAGED_AGENT_BLOBS = {
    "johnny-dqa.toml": {"109a69e6a8b7c9c35381f68987b5a8c9c53d0172"},
    "johnny-pm.toml": {
        "a3d3b70d1aee79dc53d2bd4ad01e956ce04d487f",
        "bbb1a7d066c94394a55bf43467173a0ef1ff4be2",
    },
}
...
def install_agent_templates(project: Path, *, preserve_existing: bool = False) -> None:
    ...
    for source in source_dir.glob("*.toml"):
        ...
        if target.exists():
            ...
            target.write_bytes(content)
            managed[source.name] = source_hash
    for name, legacy_blobs in RETIRED_MANAGED_AGENT_BLOBS.items():
        target = target_dir / name
        if not target.exists():
            managed.pop(name, None)
            continue
        current = target.read_bytes()
        known_managed = sha256_bytes(current) == managed.get(name)
        known_legacy = git_blob_oid(current) in legacy_blobs
        if known_managed or known_legacy:
            target.unlink()
            managed.pop(name, None)
...
def install_project_rules(project: Path) -> None:
    ...
    managed_rules = rules_source.read_text(encoding="utf-8")
    if not rules_target.exists():
        updated_rules = managed_rules
    else:
        content = rules_target.read_text(encoding="utf-8")
        if RULES_V4_START in content and RULES_V4_END in content:
            prefix, remainder = content.split(RULES_V4_START, 1)
            _, suffix = remainder.split(RULES_V4_END, 1)
            managed_block = managed_rules.split(RULES_V4_START, 1)[1].split(
                RULES_V4_END, 1
            )[0]
            updated_rules = (
                prefix.rstrip()
        ...
        elif RULES_V3_MARKER in content:
            # v3 was wholly plugin-managed and had no safe block boundary.
            updated_rules = managed_rules
        else:
            # Preserve an unmanaged project preface and append a replaceable block.
            updated_rules = content.rstrip() + "\n\n" + managed_rules
    rules_target.write_text(updated_rules, encoding="utf-8", newline="\n")
    ...
    if not manifest_target.exists():
        ...
        current["model_matrix"] = template.get("model_matrix")
        current["task_context_pack"] = template.get("task_context_pack")
        current["pm_document_root"] = template.get("pm_document_root")
        current["pm_document_directories"] = template.get(
            "pm_document_directories", []
        )
        atomic_json(manifest_target, current)


def ensure_pm_directories(project: Path) -> None:
    """Create the managed PM archive layout without moving user documents."""
    for relative in (
        "PM/Planning",
        "PM/PRD",
        "PM/Flows",
...
def migrate_config(config: dict) -> dict:
    ...
    config["phase4_commit_roots"] = ["src/"]
...
def enable(project_arg: Path) -> None:
    ...
    config.setdefault(
        "evidence_paths",
        ["TDD_DQA/", "SDD_DQA/", "Claude DQA/", "PM/", "Engineer/", "Architect/"],
    )
    ...
    config["ticket_flow"].setdefault("max_active_phase4_tickets", 1)
    ...
    config.setdefault("phase4_flow", {})
    config["phase4_flow"].update(
        {
            "purpose": "architecture-deepening",
            "architecture_skill": "improve-codebase-architecture",
            "milestone_prefix": "P4-M",
            "branch_prefix": "codex/phase4-M",
            "preserve_phase3_behavior": True,
            "allow_new_features": False,
            "architect_writes_as_built": True,
            "construction_requires_separate_ceo_approval": True,
        }
    )

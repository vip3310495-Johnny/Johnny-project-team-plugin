...
You receive bounded, redacted trace or report files explicitly supplied by the PM or user. Do not assume a trace-extraction script exists.
...
This is an optional, manually invoked observability workflow. **DO NOT modify
aggregate log files directly.**

1. Verify the supplied trace is bounded and redacted.
2. Read only the evidence needed for the current diagnosis.
3. Write output to `.johnny/observability/` or the PM-approved temporary path.
4. If `log_aggregator.py` is present in the project and documented by its own
   project rules, use it; otherwise hand the report path back to PM.
5. Record an enforceable lesson only through the formal
   `johnny_lesson_record.py` entry point.

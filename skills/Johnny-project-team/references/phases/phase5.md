...
- PM 必須第一時間去翻閱 `.agents/lessons_learned/DIGEST.md` 或查閱剛剛產出的 `Architect/As_Built_Architecture.md` 來回答 CEO。
- PM 在此階段扮演的是不折不扣的「活體知識庫」。

## 3. 觸發新迭代 (Triggering New Features)
- 當 CEO 提出新的功能需求 (New Features) 或大型改版時，這並不屬於簡單的 Bugfix。
- PM 必須重新建立一份新的 PRD，並帶領團隊**無縫發動全新的 Phase 1 (Global Architecture & PRD)** 來拆解新需求。
- **繼承知識**：PM 在 `invoke` 新的 Engineer 與 DQA 時，必須將上一代的 `.agents/lessons_learned/DIGEST.md` 餵給他們，確保新團隊不會重蹈覆轍。

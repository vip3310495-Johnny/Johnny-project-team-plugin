...
description: 以 Codex 原生協作管理多代理開發、CEO 核准、大小 Milestone、DQA 與可驗證 Phase Gate。當使用者要求跨 Phase 專案治理、核准流程、DQA 驗證、治理 Log 或專案狀態恢復時使用。
---

# Johnny Project Team（Codex）

以繁體中文協調專案。使用本 Skill 時，先讀取 `references/governance.md`。所有治理寫入必須使用 `scripts/project_governance.py`，不得以提示詞或單獨 `/approve` 字串取代。
...
8. Engineer 只可修改 `src/` 與 `tests/` 的程式碼，不得修改 DQA 目錄、DQA 報告或治理檔案；Engineer 也不得使用任何 DQA 專用工具、腳本、Gate 或 Claude DQA CLI。測試執行與 PASS／FAIL 判定一律由 DQA 執行。

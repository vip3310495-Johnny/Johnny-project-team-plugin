...
- **【鐵律】強制閘門與授權通報 (Phase Gate & /approve)**：在面臨 Phase 跳轉、Milestone 完成或需要危險權限時，PM **絕對禁止**擅自跳步或猜測 CEO 意圖。PM 必須：
  1. **強制階段閘門**：階段只能透過 `python scripts/johnny_phase_gate.py --project <repo> --to-phase <N> --approval "<明確核准內容>"` 推進。
...
- **觸發時機**：只有使用者要求額外獨立觀點時，才在 TDD 與 SDD PASS 後手動執行 `scripts/claude_dqa.py`；它必須呼叫真正的 Claude CLI，並透過共用 DQA 狀態機記錄結果。

...
- 每次訊息只提出一個待決策問題，並等待使用者回答後才可進入下一題；不得一次列出完整問卷或後續問題。
- 先用環境、檔案或工具查證可驗證的事實；只把需要使用者決策的事項提出詢問。
- 每題說明決策重點，並提供建議答案；使用者尚未決定時，提供 2-3 個互斥選項。
- 依賴前一項決策的問題必須按順序處理，不得跳過未釐清的前提。
- 完成 5W1H 共識並取得 `/approve` 前，不得開始實作或建立專案產出。
